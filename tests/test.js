    ...
    describe('default options', () => {
      it('does not try calling flash font detection', (done) => {
        ...
        Fingerprint2.get((components) => {
          expect(
            components.some(({ key, _value }) => key === flashFontKey)
          ).toBeFalsy()
          done()
        })
      })
      ...
      describe('different components', () => {
        it('can provide different components', (done) => {
          let customFunction = (done) => done('RANDOM_STRING')

          let spy = jasmine
            .createSpy('customFunction', customFunction)
            .and.callThrough()
          Fingerprint2.get(
            {
              components: [{ key: 'customFunction', getData: spy }]
            },
            (keys) => {
        ...
        it('its key is used', (done) => {
          ...
          Fingerprint2.get(options, (components) => {
            expect(getComponent(components, 'TEST_STRING')).toEqual(
              'RANDOM_STRING'
            )
            done()
          })
      ...
      describe('enumerate devices fingerprint', () => {
        it('checks enumerate devices fingerprint', (done) => {
          let options = {
            components: [Fingerprint2.components.enumerateDevices]
          }
          Fingerprint2.get(options, (components) => {
            if (
              ...
              expect(
                getComponent(components, 'enumerateDevices')
              ).not.toBeNull()
            }
            done()
          })
      ...
      describe('WebGL shader precision format', () => {
        ...
        it('checks webgl shader precision format loop', (done) => {
          ...
          let webglExpectedArray = [
            ...
            item(
              'vertex',
              'medium float precision rangeMin',
              gl.VERTEX_SHADER,
              gl.MEDIUM_FLOAT,
              'rangeMin',
            ),
            item(
              'vertex',
              'medium float precision rangeMax',
              gl.VERTEX_SHADER,
              gl.MEDIUM_FLOAT,
              'rangeMax',
            ),
          ...
          Fingerprint2.get((components) => {
            const value = getComponent(components, 'webgl')
            webglExpectedArray.forEach((item) =>
              expect(value.indexOf(item)).not.toEqual(-1),
            )
            done()
          })
        })
      ...
      describe('preprocessor', () => {
        it('checks that preprocessor not used by default', (done) => {
          Fingerprint2.get((components) => {
            expect(getComponent(components, 'userAgent')).not.toEqual(
              'MyUserAgent',
            )
            done()
          })
      ...
      describe('hasLiedOS', () => {
        it('works for iPhone', (done) => {
          ...
          Object.defineProperty(navigator, 'userAgent', {
            value:
              'Mozilla/5.0 (iPhone; CPU iPhone OS 12_1_4 like Mac OS X) AppleWebKit/605.1.15',
            configurable: true,
          })
